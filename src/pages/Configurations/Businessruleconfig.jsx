...
const EMPTY_RULE = { system: '', parameter: '', operator: '>', value: '', action: 'Raise Exception', severity: 'medium' };
...
export default function ConfigBusinessRules() {
  ...
  const [ruleLines, setRuleLines] = useState([{ ...EMPTY_RULE, id: Date.now() }]);

  /* ── Rule line management ── */
  function addRuleLine() {
    setRuleLines(prev => [...prev, { ...EMPTY_RULE, id: Date.now() }]);
  }

  function updateRuleLine(id, field, value) {
    setRuleLines(prev => prev.map(r => r.id === id ? { ...r, [field]: value } : r));
  }

  function deleteRuleLine(id) {
  ...
  function handleSave() {
    ...
    const newRules = validLines.map((r, i) => ({
      id:        Date.now() + i,
