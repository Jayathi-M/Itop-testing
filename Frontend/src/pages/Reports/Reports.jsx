...
// The 3 download format choices shown in the popup
...
// Colored pill showing report status (Published / Draft / Pending)
...
// Colored pill showing report type (Exception / Compliance / etc.)
...
// One of the 4 big number tiles at the top of the page
...
// Tab bar (Report Library | Generate New)
...
/* --- DOWNLOAD DIALOG --------------------------------------------- */
/*
  Shows when the user clicks the Download button on a report row.
  Props:
    report  - the report object that was clicked
    onClose - called when the dialog should close
*/
function DownloadModal({ report, onClose }) {
  ...
  function handleDownload() {
    ...
    // Simulate a short preparation delay
    setTimeout(() => {
      ...
      // Build a simple text file as a placeholder
      const text = [
        'Report:     ' + report.title,
        'ID:         ' + report.id,
        'Type:       ' + report.type,
    ...
    <div className="dl-backdrop" onClick={handleBackdropClick}>
      <div className="dl-modal">
        ...
        <div className="dl-modal__body">
          {done ? (
            // Success screen shown after download starts
            ...
            <div>
              ...
              {/* Preview of the exact filename that will be saved */}
        ...
        {/* Footer - hidden after download starts */}
        {!done && (
...
function ReportLibrary({ search, onDownload }) {
...
export default function ReportsPage() {
  ...
  // dlReport holds the report the user clicked Download on.
  ...
  return (
    <div className="rpt-page">
      ...
      {/* Tab switcher */}
      <Tabs
        tabs={['Report Library', 'Generate New']}
        active={activeTab}
      ...
      {/* Download dialog - rendered on top of everything when dlReport is set */}
      {dlReport && (
        <DownloadModal
          report={dlReport}
