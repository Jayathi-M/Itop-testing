...
// The 4 export formats shown in the popup
...
/* ---- EXPORT POPUP ---- */
/*
  A small popup that slides in at the top-right corner of the screen.
  Shows 4 format buttons. Clicking one builds a file and downloads it.
  Props:
    logs    - the filtered rows to export
    onClose - called to close the popup
*/
function ExportMenu({ logs, onClose }) {
  const [done, setDone] = useState(null); // stores the format id once export starts

  // Clicking the backdrop (dark area) closes the menu
  function onBackdrop(e) {
    if (e.target === e.currentTarget) onClose();
  }

  function handleExport(fmt) {
    // Build a simple CSV-style text file for all formats
    // (In a real app you'd call a backend endpoint for proper PDF/Excel generation)
    ...
    setDone(fmt.id);          // mark this format as done (shows green tick)
    setTimeout(onClose, 1800); // auto close after 1.8 seconds
  }

  return (
    // Full-screen invisible layer — clicking it closes the menu
    <div className="export-backdrop" onClick={onBackdrop}>
      {/* The actual small popup box — anchored to top-right */}
...
export default function AuditTrail() {
  ...
  // Apply all active filters to get the rows shown in the table
  ...
  // Count how many filter dropdowns are currently active (not "All" / empty)
  ...
  return (
    <div className="at-page">
      ...
      {/* Title left | search + filters + export right */}
      <div className="at-page__header">
        ...
        <div className="at-header-right">
          ...
          {/* Filters toggle button — shows a red badge when filters are on */}
          <button
          ...
          {/* Export button + popup anchored to it */}
          <div style={{ position: 'relative' }}>
            ...
            {/* Popup appears right below the button, aligned to the right edge */}
            {showExport && (
              <ExportMenu
                logs={visible}
      ...
      {/* Filter panel — slides open when Filters button is clicked */}
      {showFilters && (
