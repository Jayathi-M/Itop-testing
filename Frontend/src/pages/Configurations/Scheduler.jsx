...
function Scheduler({ system = { name: "Unknown", assets: 0 }, onStatsChange }) {
  ...
  function handleDraftStatus(id, value) {
    setSchedules(prev => prev.map(s => s.id === id ? { ...s, draftStatus: value } : s));
  }

  function handleDraftDate(id, value) {
  ...
  function handleUpdate(id) {
    setSchedules(prev =>
      prev
        ...
        .filter(s => s.status !== "completed")   // remove completed only after confirming update
    );
  }
  ...
  /* ── FIX: search runs first across ALL schedules
          hide completed ONLY when search is empty ── */
  const filteredSchedules = schedules.filter(s => {
    const q = searchTerm.toLowerCase().trim();

    // when searching — show everything including completed
    if (q) {
      return (
    ...
    // when not searching — hide completed cards (they disappear after update)
              ...
              <div className="sch-editor">
                <div className="sch-editor__row">
                  <div className="sch-field">
                    ...
                    <select
                      className="sch-field__select"
                      value={s.draftStatus}
                      onChange={e => handleDraftStatus(s.id, e.target.value)}
                    >
                  ...
                  <div className="sch-field">
                    ...
                    <input
                      className="sch-field__input"
                      type="datetime-local"
                      value={s.draftDate}
                      onChange={e => handleDraftDate(s.id, e.target.value)}
                    />
                  </div>
                </div>
          ...
          <div className="sch-modal">
            ...
            <div className="sch-modal__body">
              ...
              <div className="sch-form-group">
                ...
                <select value={formData.facility}
