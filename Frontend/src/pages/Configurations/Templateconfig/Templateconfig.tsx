...
type TemplateType = 'report' | 'checklist';

interface EditItem {
  item: ReportTemplate | ChecklistTemplate;
  type: TemplateType;
}

const REPORT_TEMPLATES_DATA: ReportTemplate[] = [
...
const CHECKLIST_TEMPLATES_DATA: ChecklistTemplate[] = [
...
function EditModal({ item, type, onSave, onClose }: { item: ReportTemplate | ChecklistTemplate; type: TemplateType; onSave: (u: ReportTemplate | ChecklistTemplate) => void; onClose: () => void }) {
  const [name,        setName]        = useState(item.name);
  const [scope,       setScope]       = useState(item.scope);
  const [format,      setFormat]      = useState((item as ReportTemplate).format || 'PDF');
  const [checkpoints, setCheckpoints] = useState((item as ChecklistTemplate).checkpoints || 0);
  const [content,     setContent]     = useState(item.content || '');

  function handleSave() {
    if (!name.trim()) return;
    const updated = type === 'report'
      ? { ...item, name: name.trim(), scope, format,      modified: new Date().toISOString().slice(0,10) } as ReportTemplate
      : { ...item, name: name.trim(), scope, checkpoints: Number(checkpoints), modified: new Date().toISOString().slice(0,10) } as ChecklistTemplate;
    onSave(updated);
  }
          ...
          <div className="form-row-2">
            ...
            {type === 'report' ? (
              ...
              <div className="form-field">
                ...
                <input className="field-input" type="number" min="0" value={checkpoints} onChange={e => setCheckpoints(Number(e.target.value))} />
              </div>
            )}
          </div>
          <div className="form-field">
            <label className="field-label">Template Content</label>
            <textarea className="field-textarea" rows={5} placeholder="Enter template structure or notes..." value={content} onChange={e => setContent(e.target.value)} />
          </div>
...
export default function ConfigTemplates() {
  const [reportTemplates,    setReportTemplates]    = useState<ReportTemplate[]>(REPORT_TEMPLATES_DATA);
  const [checklistTemplates, setChecklistTemplates] = useState<ChecklistTemplate[]>(CHECKLIST_TEMPLATES_DATA);
  const [search,             setSearch]             = useState('');
  const [editItem,           setEditItem]           = useState<EditItem | null>(null);
  const [showForm,           setShowForm]           = useState(false);
  const [newType,            setNewType]            = useState<TemplateType>('report');
  ...
  function handleEditSave(updated: ReportTemplate | ChecklistTemplate) {
    if (editItem?.type === 'report') {
      setReportTemplates(prev => prev.map(t => t.id === updated.id ? updated as ReportTemplate : t));
    } else {
      setChecklistTemplates(prev => prev.map(t => t.id === updated.id ? updated as ChecklistTemplate : t));
    }
    setEditItem(null);
  }

  function deleteTemplate(id: number, type: TemplateType) {
      ...
      <div className="ct-page__header">
        ...
        <div className="ct-header-right">
          <div className="search-box">
            ...
            <input className="search-box__input" placeholder="Search templates..." value={search} onChange={e => setSearch(e.target.value)} />
          </div>
        ...
        <div className="ct-form-card">
          ...
          <div className="form-row-3">
            ...
            {newType === 'report' ? (
              ...
              <div className="form-field">
                <label className="field-label">Checkpoints</label>
                <input className="field-input" type="number" min="1" value={formCheckpoints} onChange={e => setFormCheckpoints(Number(e.target.value))} />
              </div>
            )}
          </div>
          <div className="form-field">
