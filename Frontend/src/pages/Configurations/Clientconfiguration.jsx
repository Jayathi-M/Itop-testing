...
const PANELS = { sdk: SdkPanel, port: PortPanel, api: ApiPanel, db: DbPanel, file: FilePanel };
...
/* ══════════════════════════════
   MAIN COMPONENT
══════════════════════════════ */
const ClientConfig = ({ onStatsChange }) => {
  ...
  // Push stats up to Empower header
  React.useEffect(() => {
  ...
  return (
    <div className="cc">
      ...
      {/* ── Integration type chips ── */}
      <div className="cc__chips">
        {TYPES.map(t => (
          <button
            key={t.id}
            className={`cc__chip ${activeType === t.id ? "cc__chip--active" : ""}`}
