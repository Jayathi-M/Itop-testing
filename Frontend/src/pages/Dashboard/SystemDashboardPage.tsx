...
function LineGraph() {
  ...
  function toX(i: number) { return PL + (i / steps) * gW }
  function toY(v: number) { return PT + gH - (v / maxV) * gH }
...
export default function SystemDashboard() {
  ...
  const [filters,     setFilters]     = useState<FilterState>({ location:'Mumbai', plant:'Unit-1', module:'Empower' })
  ...
  function Filter({ label, k, options }: { label: string; k: keyof FilterState; options: string[] }) {
    return (
        ...
        <>
          <div className="sd__filter-bar">
            ...
            <div className="sd__filters">
              <Filter label="Location" k="location" options={['Hyderabad','Mumbai','Chennai','Calcutta']} />
              <Filter label="Plant-1"  k="plant"    options={['Unit-1','Unit-2']} />
              <Filter label="Module"   k="module"   options={['Empower','Chromeleon','Lab Solutions']} />
            </div>
          </div>
